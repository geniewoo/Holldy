$(function() {
    $(document).on('click', '#files_send', function() {
        ...
        if ($('#commWriteName').val() == ''){
            alert('작성자의 성함을 입력해 주세요');
            return;
        }else if (!(/^[가-힣]{2,4}$/).test($('#commWriteName').val())) {
            alert('이름을 한글 2~4자로 작성해 주세요');
            return;
        }
        if($('#commWriteLocked').is(':checked')){
            if(!(/^[a-zA-Z0-9]{4,10}$/).test($('#commWritePassword').val())){
                alert('비밀번호를 영어, 숫자조합 4~10자로 작성해 주세요');
                return;
            }
        }
        if ($('#photo_upload')[0].files.length > 0) {
                ...
                success: function(data) {
                    ...
                    if (data.code === 1) {
                        ...
                        var postData = {
                            ...
                            commWriteName: commWriteName
                        };
                        if($('#commWriteLocked').is(':checked')){
                            postData.locked = true;
                            postData.password = $('#commWritePassword').val();
                        }else{
            ...
            var commWriteCont = $('#commWriteCont').val();
            var commWriteName = $('#commWriteName').val();
            var commWritePassword = $('#commWritePassword').val();
            var postData = {
                commWriteTitle: commWriteTitle,
                commWriteCont: commWriteCont,
                commWriteName: commWriteName
